<nav x-data="{ open: false }" class="bg-gray-900 border-b border-gray-700 text-gray-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @if(Auth::check() && strtolower(Auth::user()->rolename ?? '') !== 'praktijkmanagement')
                        @php
                            $roleRoute = strtolower(Auth::user()->rolename ?? '');
                        @endphp
                        <x-nav-link :href="route($roleRoute . '.index')" :active="request()->routeIs($roleRoute . '.index')">
                            {{ __(ucfirst($roleRoute)) }}
                        </x-nav-link>
                    @endif
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-gray-600 text-sm leading-4 font-medium rounded-md text-gray-100 bg-gray-800 hover:bg-gray-700 focus:outline-none transition ease-in-out duration-150">
        ...
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if(Auth::check() && strtolower(Auth::user()->rolename ?? '') !== 'praktijkmanagement')
                @php
                    $roleRoute = strtolower(Auth::user()->rolename ?? '');
                @endphp
                <x-responsive-nav-link :href="route($roleRoute . '.index')" :active="request()->routeIs($roleRoute . '.index')">
                    {{ __(ucfirst($roleRoute)) }}
                </x-responsive-nav-link>
            @endif
